...
export default function SignupForm() {
  ...
  const requestSignupCode = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setMessage("קוד אימות נשלח למייל.");
        setStep("verify");
      } else {
        setMessage(data.message || "שגיאה בשליחת הקוד");
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4 fade-in">
      <CardSimple className="max-w-md w-full p-6 space-y-6">
        <h2 className="text-2xl font-bold text-center">הרשמה</h2>

        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (step === "form") requestSignupCode();
            else if (step === "verify" && code.length === 6) confirmSignup();
          }}
          className="space-y-4"
        >
          {step === "form" ? (
            <>
              ...
              <Input name="password" type="password" placeholder="סיסמה" value={form.password} onChange={handleChange} required />
              <Input name="phone" placeholder="טלפון (אופציונלי)" value={form.phone} onChange={handleChange} />
            </>
          ) : (
            <>
              <p className="text-sm text-center text-muted-foreground">
                קוד אימות נשלח אל: <b>{form.email}</b>
              </p>
              <Input
                type="text"
                maxLength={6}
                ...
                autoComplete="one-time-code"
              />
              <Button type="button" variant="ghost" onClick={requestSignupCode} disabled={loading}>
                שלח קוד מחדש
              </Button>
            </>
          )}
