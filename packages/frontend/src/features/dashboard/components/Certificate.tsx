...
//   if (loading) {
//     return <div className="text-center py-8">טוען תוכן תעודה...</div>;
//   }
...
//   const titleItem = contents.find(item => item.key_name === "certificate_title");
//   const descItem = contents.find(item => item.key_name === "certificate_desc");
...
//         {titleItem ? titleItem.content : "תעודת מוכנות לראיון"}
//       </h1>
...
export const Certificate: React.FC<CertificateProps> = ({ first_name }) => {
  const certificateRef = useRef<HTMLDivElement>(null);
  ...
  return (
    ...
    >
      ...
      <p className="text-base text-[--color-secondary-text]">מוענקת ל־</p>
      <h2 className="text-2xl font-semibold text-[--color-primary]">{first_name}</h2>
