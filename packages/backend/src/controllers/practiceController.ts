...
import { getQuestionsByTopic, checkAnswer } from '../reposioty/practiceRepository';

export const getQuestionsByTopicController = async (req: Request, res: Response) => {
  const { topic_id } = req.params;
  try {
    const questions = await getQuestionsByTopic(topic_id);
    res.json({ questions });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'שגיאה בשליפת השאלות' });
  }
};
...
export const submitAnswerController = async (req: Request, res: Response) => {
  ...
  try {
    const isCorrect = await checkAnswer(question_id, answer);
    res.json({ correct: isCorrect });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'שגיאה בבדיקת התשובה' });
  }
};
