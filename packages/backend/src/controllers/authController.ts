...
export const generateAndSendCode = async (req: Request, res: Response) => {
  ...
  console.log("[generateAndSendCode] Received email:", email);
  if (!email) {
    console.log("[generateAndSendCode] Missing email in request");
    return res.status(400).json({ sent: false, message: "Email is required" });
  }
  const code = Math.floor(100000 + Math.random() * 900000).toString();
  const expiresAt = Date.now() + 5 * 60 * 1000;
  codesPerEmail.set(email, { code, expiresAt });
  console.log(`[generateAndSendCode] Generated code ${code} for email ${email}, expires at ${new Date(expiresAt).toISOString()}`);

  await sendVerificationCodeEmail(email, `קוד האימות שלך הוא: ${code}`);
  console.log(`[generateAndSendCode] Verification code sent to ${email}`);
...
export const forgotPassword = async (req: Request, res: Response) => {
  const { email } = req.body;
    console.log('Forgot Password requested for email:', email); // הוספת לוג
