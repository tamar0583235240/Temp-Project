    ...
    const { firstName, lastName, email, phone, role, isActive, password } =
      userData;
        ...
        const res = await pool.query(`
            UPDATE users 
            SET firstName = $1, lastName = $2, email = $3, phone = $4, role = $5, is_active = $6, password = COALESCE($7, password)
...
const createUser = async (user: Users): Promise<Users> => {
    try {
        ...
        const res = await pool.query(
            `INSERT INTO users (id, firstName, lastName, email, phone, role, created_at, is_active, password)
...
const insertUser = async (user: {
  ...
  const result = await pool.query(
    `INSERT INTO users (id, firstName, lastName, email, phone, role, is_active, password, created_at)
