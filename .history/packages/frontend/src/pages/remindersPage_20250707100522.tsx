...
// }
export default function RemindersPage() {
  const userId = "13a8730d-6247-4988-8fa1-a28abd6fb131"; // מהדוגמה שלך
  ...
  const handleOptionChange = (type: string, frequency: string | null) => {
    setSelections((prev) => ({ ...prev, [type]: frequency }));
  };

  const handleSaveAll = async () => {
    const payload = Object.entries(selections)
      .filter(([_, v]) => v !== null)
      .map(([type, frequency]) => ({ type, frequency: frequency! }));

    if (payload.length === 0) return alert("לא נבחרו תזכורות");
